...
ドキュメントの記載どおり1リポジトリあたり10本でした（2026-08-26 実測）。ここは記載と実装が一致しています。

上限に達すると、作成を試みてエラーになるのではなく `New rule` ボタン自体が非活性になります。GitHub presets の2本は Repository rules の10本とは別枠で、上限にはカウントされません。

カウントされるのは Enabled のルールだけではありません。10本埋まった状態で1本を `Disabled` に切り替えても、`New rule` は非活性のままでした。ルールが存在するだけで枠を消費します。

つまり「使わなくなったルールを無効化して保管しておく」ことはできません。枠を空けるには削除するしかない。10本という上限は、見た目より厳しく効きます。
